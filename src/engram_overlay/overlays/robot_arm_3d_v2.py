...
def bilinear_point(
    ...
    """Interpolate a projected quad ordered around its perimeter."""
...
def v2_surface_faces(base: Vec3, joints: list[Vec3]) -> list[Face3D]:
    ...
    faces.extend(sphere_faces(joints[-1], 38.0, color=PLAIN_WHITE, rings=5, segments=12, z_scale=0.76))
    faces.extend(sphere_faces(joints[-1], 31.0, color=PLAIN_JOINT, rings=4, segments=10, z_scale=0.72))
    return faces
...
class RobotArm3DV2View(RobotArm3DView):
    """V1 behavior with denser reference-inspired geometry and sampled surfaces."""
    ...
    def _draw_projected_face(self, face: Face3D, coordinates: tuple[float, ...]) -> None:
        ...
        corners = tuple((coordinates[index], coordinates[index + 1]) for index in range(0, 8, 2))
        ...
        for row in range(self.TEXTURE_ROWS):
            v0 = row / self.TEXTURE_ROWS
            v1 = (row + 1) / self.TEXTURE_ROWS
            for column in range(self.TEXTURE_COLUMNS):
                u0 = column / self.TEXTURE_COLUMNS
                u1 = (column + 1) / self.TEXTURE_COLUMNS
                cell = (
                    bilinear_point(corners, u0, v0),
                    bilinear_point(corners, u1, v0),
                    bilinear_point(corners, u1, v1),
                    bilinear_point(corners, u0, v1),
                )
                sample_u = (u0 * 0.72 + phase * 0.28) % 1.0
                sample_v = (v0 * 0.72 + (1.0 - phase) * 0.28) % 1.0
                fill = self.texture_atlas.color(face.color, sample_u, sample_v, intensity)
                self.canvas.create_polygon(
                    *(coordinate for point in cell for coordinate in point),
                    fill=fill,
                    outline=fill,
                    width=1,
                    tags=("scene3d",),
